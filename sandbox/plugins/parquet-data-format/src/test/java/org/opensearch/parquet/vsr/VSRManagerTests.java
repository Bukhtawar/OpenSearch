...
public class VSRManagerTests extends ParquetBaseTests {
    ...
    /**
     * Lazy LIST materialization: a declared-LIST column that only ever saw singletons is NOT
     * canonicalized when the VSR freezes while the file shape is still undecided — the batch
     * exports in its scalar physical shape, so an all-singleton file is a plain scalar file.
     */
    public void testAllSingletonColumnStaysScalarAtFreeze() throws Exception {
        ...
        try {
            ...
            // Freeze directly (bypassing flush) to pin where the shape decision lands.
            active.moveToFrozen();
            Field tagsField = active.getSchema().findField("tags");
            assertEquals("undecided file shape must freeze scalar, not canonicalize", new ArrowType.Utf8(), tagsField.getType());
        } finally {
            manager.close();
        }
    }
    ...
    /**
     * Once the file schema is locked scalar (first batch exported without any array), a
     * document carrying several values is rejected recoverably — the file can never hold an
     * array — and singleton ingestion continues unaffected. The retry destination is a new
     * writer generation, whose fresh latch starts undecided.
     */
    public void testArrayDocRejectedWhenFileLockedScalar() throws Exception {
        String filePath = createTempDir().resolve("locked-scalar-reject.parquet").toString();
        // Rotation threshold 2: the third addDocument freezes+exports the first batch, which
        // initializes the native writer and locks the file shape scalar.
        VSRManager manager = new VSRManager(filePath, indexSettings, schema, bufferPool, 2, threadPool, 0L);
        try {
            manager.reconcileSchema(schemaWithMultiValue("tags"));
            KeywordFieldMapper.KeywordFieldType tags = new KeywordFieldMapper.KeywordFieldType("tags");
            tags.setMultiValued(true);
            ...
            // File shape is now locked scalar: an array doc must reject recoverably.
            ParquetDocumentInput arrayDoc = new ParquetDocumentInput();
            populateMetadataFields(arrayDoc);
            arrayDoc.setRowId(DocumentInput.ROW_ID_FIELD, 3);
            arrayDoc.addField(valField, 3);
            arrayDoc.addField(tags, "a");
            arrayDoc.addField(tags, "b");
            MismatchedInputException e = expectThrows(MismatchedInputException.class, () -> manager.addDocument(arrayDoc));
            assertTrue(e.getMessage().contains("locked to scalar shape"));
            assertEquals("rejected doc must not be admitted", 3, manager.getAcceptedRows());
            assertTrue("column must remain scalar after the rejection", manager.getActiveManagedVSR().isPromotable("tags"));

            // Singleton ingestion continues at the same row index.
            ParquetDocumentInput doc3 = new ParquetDocumentInput();
            populateMetadataFields(doc3);
            doc3.setRowId(DocumentInput.ROW_ID_FIELD, 3);
            doc3.addField(valField, 3);
            doc3.addField(tags, "v3");
            manager.addDocument(doc3);
    ...
    /**
     * Once the file schema is locked LIST (first batch contained an array), later batches
     * whose column never promoted are canonicalized at freeze so every batch of the file has
     * one shape — the flush succeeds end to end through the native writer.
     */
    public void testPromotedFileCanonicalizesLaterScalarBatches() throws Exception {
        String filePath = createTempDir().resolve("locked-list-canonicalize.parquet").toString();
        VSRManager manager = new VSRManager(filePath, indexSettings, schema, bufferPool, 2, threadPool, 0L);
        try {
            manager.reconcileSchema(schemaWithMultiValue("tags"));
            KeywordFieldMapper.KeywordFieldType tags = new KeywordFieldMapper.KeywordFieldType("tags");
            ...
            // Batch 1: an array doc promotes the column, so the file locks LIST at export.
            ParquetDocumentInput doc0 = new ParquetDocumentInput();
            populateMetadataFields(doc0);
            doc0.setRowId(DocumentInput.ROW_ID_FIELD, 0);
            doc0.addField(valField, 0);
            doc0.addField(tags, "a");
            doc0.addField(tags, "b");
            manager.addDocument(doc0);
            ...
            doc1.setRowId(DocumentInput.ROW_ID_FIELD, 1);
            doc1.addField(valField, 1);
            doc1.addField(tags, "solo");
            manager.addDocument(doc1);

            // Batch 2 (fresh VSR, scalar seed): singletons only — must canonicalize at freeze
            // to match the LIST file schema, or the native write would fail.
            for (int i = 2; i < 4; i++) {
                ParquetDocumentInput doc = new ParquetDocumentInput();
                populateMetadataFields(doc);
                doc.setRowId(DocumentInput.ROW_ID_FIELD, i);
