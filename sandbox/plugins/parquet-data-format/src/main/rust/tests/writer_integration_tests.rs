...
fn test_complete_writer_lifecycle() {
    ...
    assert_eq!(metadata.metadata.num_rows, 9); // 3 batches × 3 rows
    assert!(metadata.metadata.version > 0);
    assert_eq!(metadata.metadata.schema.len(), 3); // root + 2 fields
    ...
    assert_eq!(read_metadata.num_rows(), metadata.metadata.num_rows);
    assert_eq!(read_metadata.version(), metadata.metadata.version);
}
            ...
            if NativeParquetWriter::create_writer(filename.clone(), schema_ptr).is_ok() {
                ...
                if write_ok {
                    if let Ok(Some(metadata)) = NativeParquetWriter::finalize_writer(filename.clone()) {
                        if metadata.metadata.num_rows == 3
