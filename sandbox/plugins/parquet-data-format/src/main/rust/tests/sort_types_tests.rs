...
fn test_merge_sort_by_int64() {
    ...
    merge_sorted(&files, &output, "test", &["val".into()], &[false], &[false], 0, &[]).unwrap();
...
fn test_merge_sort_by_float64_with_nulls() {
    ...
    let output = tmp.path().join("merged.parquet").to_string_lossy().to_string();
    merge_sorted(&files, &output, "test", &["val".into()], &[false], &[true], 0, &[]).unwrap();
...
fn test_merge_sort_descending() {
    ...
    let output = tmp.path().join("merged.parquet").to_string_lossy().to_string();
    merge_sorted(&files, &output, "test", &["val".into()], &[true], &[false], 0, &[]).unwrap();
