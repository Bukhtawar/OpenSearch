...
fn test_finalize_writer_success() {
    ...
    assert_eq!(finalize_result.metadata.num_rows, 0);
    assert!(finalize_result.metadata.version > 0);
    assert!(!WRITER_MANAGER.contains_key(&filename));
    assert!(FILE_MANAGER.contains_key(&filename));
...
fn test_finalize_writer_with_data_returns_correct_metadata() {
    ...
    assert_eq!(metadata.metadata.num_rows, 6);
    assert!(metadata.metadata.version > 0);
    assert_eq!(metadata.metadata.schema.len(), 3); // root + 2 fields (id, name)
...
fn test_streaming_crc32_matches_reread_crc32_with_data() {
    ...
    assert_eq!(finalize_result.metadata.num_rows, 9);
    ...
    assert_eq!(
        ...
        streaming_crc32, reread_crc32, finalize_result.metadata.num_rows
    );
