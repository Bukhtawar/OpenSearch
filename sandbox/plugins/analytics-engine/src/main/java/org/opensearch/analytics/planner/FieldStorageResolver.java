...
public class FieldStorageResolver {
    ...
    private static FieldStorageInfo resolveField(
        ...
        // POC (Lucene-style auto multi-value): keyword fields are always LIST columns and typed
        // ARRAY in the Calcite schema (OpenSearchSchemaBuilder); the capability type must agree,
        // or every per-field capability lookup (scan, filter) would check the element type against
        // ops registered for ARRAY — e.g. `tags = 'x'` resolves to ARRAY_CONTAINS, which is
        // declared for FieldType.ARRAY, not KEYWORD.
        FieldType capabilityType = "keyword".equals(fieldType) || Boolean.TRUE.equals(fieldProps.get("multi_value"))
            ? FieldType.ARRAY
